...
대화에 미완료 작업이 있으면 최종 답변 끝에 **남은 작업**을 짧게 표시한다.
사용자가 이전 대화를 다시 읽지 않아도 원래 목적, 현재 상태와 다음 단계를 알 수 있도록 작업별 한두 문장으로 요약한다.

- 다른 주제로 전환해도 사용자가 취소하거나 대체하지 않은 기존 작업은 유지한다.
- 검토 중, 검토 완료·구현 전, 구현 확정·진행 중, 사용자 입력 대기를 구분한다. 제안만 한 작업을 구현이 확정된 것으로 쓰지 않는다.
- 완료하거나 취소한 작업은 목록에서 제외한다. 남은 작업이 없으면 이 표시를 생략한다.
...
변경할 저장소와 실행 환경, 권한을 확인하고 서로 다른 작업의 변경을 구분한다.

- 커밋·푸시·상태 확인·승인된 배포와 실행은 해당 작업의 후속 절차로 직접 처리한다.
- 다른 저장소나 서버라는 이유만으로 새 세션을 만들지 않는다. 순차적으로 안전하게 처리할 수 있으면 직접 수행한다.
- 다른 세션이 해당 저장소의 작업과 운영 문맥을 이미 가지고 있으면 그 세션으로 넘긴다.
- 변경을 독립된 커밋이나 PR로 분리해야 하거나, 작업 충돌을 격리하거나, 결과를 계속 추적해야 하면 위임한다. 지속적인 작업 추적에는 `orchestration`을 사용한다.
- 위임할 때는 맡길 범위와 기대하는 이득을 한 문장으로 설명한다. 스킬의 위임 권고도 이 기준을 따른다.

위임과 새 worktree가 필요한 경우에는 다음 규칙을 따른다.

- Orca 작업을 배분한 코디네이터는 Run, Task와 Dispatch를 만들고 `worker_done` 또는 `escalation`까지 결과를 추적한다.
- 여러 저장소의 작업을 배분할 때는 저장소별 Task로 나누고 의존 관계를 명시한다.
...
GitHub PR의 제목과 본문은 한국어를 기본으로 작성한다.
저장소 규칙이나 사용자가 다른 언어를 명시하면 해당 지시를 우선한다.
...
`SKILL.md`와 참조 문서는 사용자와 에이전트가 함께 읽고 유지보수하는 문서로 작성한다.
핵심 목표와 진행 순서를 먼저 보여주고, 실행 지시와 참조 자료는 사용하는 단계에 배치한다.

- 관련 문장을 하나의 문단으로 묶고, 주제나 작업 단계가 바뀌면 문단을 나눈다.
- 원문은 의미 단위로 줄을 나누고, 렌더링에서는 문단 사이의 빈 줄로 구조를 드러낸다.
- 순서가 있는 작업은 번호 목록으로, 병렬 항목은 목록으로, 같은 속성을 비교할 때는 표로 쓴다.
