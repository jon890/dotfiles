...
핵심 이유: `Write`/`Edit` 로 임시 파일에만 저장하면 사용자 화면에는 도구 호출만 보이고 내용이 숨겨져, 검토·수정 지시를 할 수 없다. 그래서 본문을 **채팅 인라인 + 실제 렌더링 HTML 두 형태로 함께** 보여준다.
...
- **인라인 + HTML 을 함께 띄운다.** 인라인은 본문 기록용, HTML 은 실제 렌더링 검토용. 사용자가 따로 요청하지 않아도 HTML 을 생략하지 않는다.
...
- 한국어 표현 정책 자가 점검 4항목 — `~/.claude/rules/korean-style.md`. 외래어 잔존(매핑표 밖 포함, 예: 케이던스→주기), 비유어·등급어(숙제를 해소/승급/강등), 명사형 종결, 생소한 한자어(순단 등)
...
`~/.claude/templates/` 는 Claude Code 자동 인식 경로가 아니라 절대경로로 참조하는 관례 폴더다. 미리보기 유틸이 더 늘면 이 skill 번들 안(`templates/` + `scripts/`)으로 옮겨 자체 완결 구조로 승격한다. 단 현재 템플릿은 brain-add skill 과 공유 중이라, 옮길 때 참조처를 함께 갱신해야 한다.
