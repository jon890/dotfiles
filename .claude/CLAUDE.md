...
### 저장소가 다르면 위임 필요성을 판단한다

기본적으로 현재 세션에서 작업을 끝낸다.
다른 저장소나 서버라는 이유만으로 새 세션을 만들지 않는다.

- 단순 조회와 안전하게 이어서 처리할 수 있는 커밋, 푸시, 배포와 실행은 현재 세션에서 처리한다.
- 다른 세션이 해당 저장소의 작업과 운영 문맥을 이미 가지고 있으면 그 세션으로 넘긴다.
- 변경을 독립된 커밋이나 PR로 분리해야 하거나, 브랜치 충돌을 격리하거나, 결과를 계속 추적해야 하면 `orchestration`을 사용한다.
- 넘길 때 담당 범위, 기대하는 이득, 선행 결과와 결정하지 못한 사항을 함께 적는다.

공유 저장소가 다른 작업 브랜치에 체크아웃되어 있거나 여러 저장소의 변경이 서로 다른
검토 단위라면 별도 worktree로 분리한다. 현재 세션에서 순차적으로 안전하게 끝낼 수 있는
후속 작업까지 기계적으로 분리하지 않는다.
