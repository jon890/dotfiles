...
대화형 스킬 (사용자 의도가 매번 다른 스킬 — weekly-report / dooray-comment-reply / blog-post-writer 등) 에 적용한다.
사용자가 "문제 없음"을 선택하면 스킬 수정 없이 종료한다.

**예외 — 자체 회고 스킬은 피드백 질문 생략**:
내부에 자동 회고 단계가 내장된 자율 실행 스킬은 사용 후 사용자 피드백 질문을 **묻지 않는다**.
이미 스킬 내부 회고가 개선 패턴을 자동 반영하므로 글로벌 피드백 질문은 중복이고, 자율 구현이라 사용자가 매 실행마다 평가할 여지가 적다.

- 대상 예: `build-with-teams` (9단계 review 회고 — critic / code-reviewer / docs-verifier 지적을 common-pitfalls / code-review-pitfalls / planning 영향 표에 자동 반영).
- 판정 기준: 스킬 SKILL.md 에 "회고" / "self-improve" / "pitfalls 갱신" 단계가 명시돼 있으면 자체 회고 스킬로 간주 → 피드백 질문 생략.
- 단 사용자가 실행 중 명시적으로 문제를 제기하면 그 건은 평소대로 SKILL.md 에 반영한다.
...
Dooray 업무·댓글 본문의 미리보기를 사용자가 요청하면 (또는 본문이 길어 채팅 인라인만으로 검토가 어려우면)
실제 Dooray 렌더링과 동일한 HTML 미리보기를 만들어 브라우저로 띄운다.
...
- HTML 미리보기를 띄워도 채팅 인라인 본문 표시 원칙은 유지한다 (인라인이 1차, HTML 은 보조).
