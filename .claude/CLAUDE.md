...
### Dooray 본문 HTML 미리보기 (템플릿 재사용)

Dooray 업무·댓글 본문의 미리보기는 **기본으로 HTML 로 띄운다** — 사용자가 따로 요청하지 않아도, 등록·게시 전 미리보기 단계에서 실제 Dooray 렌더링과 동일한 HTML 미리보기를 만들어 브라우저로 띄운다.
채팅 인라인 본문 표시만으로 미리보기를 끝내지 않는다 (사용자 피드백 2026-06-16 — "미리보기를 채팅 세션 말고 HTML 로").

- 원리: Dooray 는 본문을 NHN TOAST UI Editor 로 렌더링한다.
  같은 viewer CSS/JS (`uicdn.toast.com/editor/latest`) 를 쓰면 실제 등록 화면과 거의 동일하게 보인다.
- 템플릿·생성기: `~/.claude/templates/dooray-preview/` (template.html + generate.py)
- 사용법:
...
- 주의: 본문에 `</script>` 문자열 금지 (생성기가 검출·거부). CDN 로드라 오프라인이면 스타일이 빠진다.
- 채팅 인라인 본문 표시와 HTML 미리보기를 **함께** 띄운다 — 인라인은 본문 기록용, HTML 은 실제 렌더링 검토용. HTML 띄우기를 생략하지 않는다.

### GitHub issue/PR 본문 HTML 미리보기 (템플릿 재사용)

GitHub issue·PR 본문의 미리보기는 **기본으로 HTML 로 띄운다** — 사용자가 따로 요청하지 않아도, 등록·게시 전 미리보기 단계에서 GitHub 렌더링과 비슷한 HTML 미리보기를 만들어 브라우저로 띄운다.
채팅 인라인 본문 표시만으로 미리보기를 끝내지 않는다 (사용자 피드백 2026-06-16).

- 원리: GitHub 는 본문을 GitHub Flavored Markdown 으로 렌더링한다.
  github-markdown-css (GitHub 공식 마크다운 스타일) + marked.js 를 쓰면 실제 화면과 비슷하게 보인다.
- 템플릿·생성기: `~/.claude/templates/github-preview/` (template.html + generate.py)
- 사용법:
...
- `--type` 은 `issue` 또는 `pr` (헤더 배지 색 구분).
- 주의: 본문에 `</script>` 문자열 금지 (생성기가 검출·거부). CDN 로드라 오프라인이면 스타일이 빠진다.
- GitHub 고유 자동링크 (`#번호`) 와 `:emoji:` 코드는 marked.js 가 변환하지 않는다. 정확한 GFM 은 등록 후 GitHub 에서 확인한다.
- 채팅 인라인 본문 표시와 HTML 미리보기를 **함께** 띄운다 — 인라인은 본문 기록용, HTML 은 실제 렌더링 검토용. HTML 띄우기를 생략하지 않는다.

> 두 미리보기 생성기가 든 `~/.claude/templates/` 는 Claude Code 가 자동 인식하는 공식 경로가 아니라 절대경로로 참조하는 관례 폴더다.
